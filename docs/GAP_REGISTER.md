...
| G-42 | Marketing Lighthouse step is flaky: the preview server at `127.0.0.1:4173` intermittently fails to accept connections, producing `CHROME_INTERSTITIAL_ERROR`. Observed on PR #4 and PR #6; on PR #6 the same commit failed the `push` run and passed the `pull_request` run, and a re-run of the failed job passed. Distinct from the score flake closed by median-of-3 — all three runs hit the same dead server. A flaky **required** check trains contributors to re-run until green, which is gate bypass by habit (WO-002A: "a failing gate cannot be bypassed") | 5 | OPEN | `ci.yml` "Verify marketing routes and responsive experience"; run 35476977527 failed then succeeded unchanged. Fix: wait-for-port readiness gate before Lighthouse, not a retry | WO-002A AC-09 (must be stable before it becomes required on `main`) |
