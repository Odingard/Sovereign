...
| **Identity** | Google Cloud Identity Platform (multi-tenant, OIDC/SAML, MFA) | **RESEARCH / NOT APPROVED FOR PHI** | Covered under Google GCP BAA (Google HIPAA implementation guide published) | US only | Authentication records only; no clinical data | Target: Dev/Staging (synthetic) → Prod (post-gate) | Selected cloud IdP (ADR-0012). Identity only (ADR-0010); one IP tenant per Sovereign tenant; MFA enforced; no PHI in user metadata; audit logs to SIEM. |
| **Identity** | Auth0 (Organizations) | **RESEARCH / NOT APPROVED FOR PHI** | Requires vendor BAA (enterprise plan) | US region only | Authentication events only | Only on customer contractual demand | Alternative adapter only (ADR-0012). |
| **Identity** | Keycloak (Docker, local dev / CI) | **APPROVED FOR SYNTHETIC USE ONLY** | N/A (local) | Local host | Synthetic users only | Local Dev, CI | Implements the same IdentityProviderPort (ADR-0012); production fallback only by new ADR. |
| **Crypto** | Google Cloud KMS / Cloud EKM | **RESEARCH / NOT APPROVED FOR PHI** | Covered under Google GCP BAA | us-central1 | Key material never leaves KMS/HSM | Target: Dev/Staging/Prod | Per-tenant KEK; HSM protection level in prod; BYOK/EKM for enterprise tenants; rotation 90 days. |
| **Storage** | Google Cloud Storage | **RESEARCH / NOT APPROVED FOR PHI** | Covered under Google GCP BAA | us-central1 | Customer-managed; retention-locked audit anchors | Target: Dev/Staging/Prod | CMEK + client-side per-tenant encryption; uniform access; no public buckets. |
| **CI/CD** | GitHub (repo, Actions, WIF) | **APPROVED FOR SYNTHETIC USE ONLY** | No BAA (holds no PHI) | Global | Source and CI logs only | All | No PHI, secrets, or real identifiers in repo, issues, or CI output; gitleaks + synthetic scanner required. |
| **CI/CD** | Terraform state (GCS backend) | **APPROVED FOR SYNTHETIC USE ONLY** | Covered under Google GCP BAA | us-central1 | State may contain resource names only | All | Versioned, CMEK; no secrets in state. |
| **Telemetry** | Sentry (any plan) | **PROHIBITED for PHI-capable apps** | No BAA on file | Global | Retains payloads | `apps/marketing-site` only (no PHI) | Must never be imported by gateway, services, admin/clinician/operations apps; architecture test enforces. |
| **Security** | OWASP ZAP, Semgrep, Trivy, Checkov, Conftest, Syft, Cosign (CI tooling) | **APPROVED FOR SYNTHETIC USE ONLY** | N/A (runs in CI on synthetic targets) | CI | Scan reports only | CI, Dev, Staging | Scan targets carry synthetic data only; reports contain no PHI. |
