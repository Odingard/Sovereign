...
| **Identity** | Auth0 (Organizations, enterprise OIDC/SAML, SCIM) | **RESEARCH / NOT APPROVED FOR PHI** | Requires signed vendor BAA (enterprise plan) | US tenant region only | Authentication events only; no clinical data | Target: Dev/Staging (synthetic) → Prod (post-gate) | Identity only (ADR-0010); roles/grants stay in Sovereign; MFA enforced; log streaming to SIEM; no PHI in user metadata. Fallback: self-hosted Keycloak. |
| **Identity** | Keycloak (self-hosted on Cloud Run) | **RESEARCH / NOT APPROVED FOR PHI** | N/A (self-hosted; GCP BAA covers infra) | us-central1 | Customer-managed | Fallback only | Same restrictions as above; operational burden accepted only if Auth0 BAA terms fail. |
