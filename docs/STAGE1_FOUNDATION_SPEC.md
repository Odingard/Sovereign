...
| S1-D07 | Identity provider: adapter behind ADR-0010 ports. **Google Cloud Identity Platform** in cloud environments (under the GCP BAA; one IP tenant per Sovereign tenant), **Keycloak** in local dev/CI. Auth0 only on customer demand (ADR-0012). | No second vendor BAA; pay-as-you-go; identity stays separate from authority. |
...
| IdP | Keycloak (docker) + Identity Platform `sovereign-dev` | Identity Platform `sovereign-staging` | Identity Platform `sovereign-prod` |
...
### 4.1 IdP configuration (Identity Platform in cloud, Keycloak locally; port-neutral)
- One Identity Platform **tenant** (Keycloak **realm** locally) per Sovereign tenant. Tenant metadata: `tenant_id` (generated by Sovereign, never by the IdP).
...
- Blocking function (Identity Platform) / protocol mapper (Keycloak) adds custom claims:
...
- Login (Identity Platform hosted or embedded sign-in; Keycloak locally), tenant picker if multi-tenant.
